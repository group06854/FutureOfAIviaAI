...
The effectiveness of the VGAE model was evaluated on the SemanticGraph dataset using a standardized pipeline and the AUC-ROC metric. The model demonstrates balanced results, occupying an average position among all tested methods (M1-M8). For problems predicting strong bonds (edge_weight=3) over a short horizon (delta=1), VGAE shows high quality (AUC up to 0.96), although it is inferior to the absolute leaders (M1, M2), while surpassing a number of models based on transformers and traditional embeddings. The key differentiating advantage of VGAE is its exceptional stability when changing graph parameters (cutoff): unlike models such as Node2Vec (M7A), which demonstrates a catastrophic drop in performance at delta=3, VGAE maintains predictably high performance in all configurations, without having "failure" scenarios. The main architectural limitation of the model is its high sensitivity to an increase in the prediction horizon (delta), where the sharpest decrease in AUC is observed among all comparable methods, indicating retraining for short-term structural patterns. Thus, VGAE sets a new reliability standard for short-term forecasting in dynamic graphs, effectively filling the methodological niche of stable graph autoencoders, but requires further modification to improve long-term generalizing ability.
